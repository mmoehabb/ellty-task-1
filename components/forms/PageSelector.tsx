...
export function PageSelector() {
  ...
  const allSelected = useMemo(() => !pages.map(page => selectedList.includes(page)).includes(false), [selectedList])
    ...
    <div className={cn(
      ...
      {pages.map((page, i) =>
        <Checkbox 
          ...
          onChange={() => {
            if (selectedList.includes(page)) return setSelectedList(prev => prev.filter(p => p !== page));
